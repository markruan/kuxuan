        ...
        factory(window['H'] = {
            ...
            dateTip: function(element, okCallback, cancelCallback) {
                ...
                var yearStartX = 0,
                    ...
                    dayStartY = 0;
                // 当前Y轴位移
                var yearCurrentTranslateY = 0,
                    monthCurrentTranslateY = 0,
                    dayCurrentTranslateY = 0;
                // 每一个列表项目高度
                var itemHeight = 45;
                // 默认下移两个列表项目高度
                ...
                // 当前索引
                var yearCurrentSelectIndex = 0,
                    monthCurrentSelectIndex = 0,
                    dayCurrentSelectIndex = 0;
                // 当前选择的时间
            ...
            toastCore: function(iconHTML, tipText, duration, animateName) {
                ...
                duration = Math.abs(that.isNumber(duration) ? Number(duration) : 2000);
                animateName = animateName ? animateName : "zoomIn";

                if (that.Ds("#H-toast-tip") && that.Ds("#H-toast-tip").length > 0) {} else {
                    var toastDiv = document.createElement("div");
                    toastDiv.id = "H-toast-tip";
                    that.addClass(toastDiv, "H-position-absolute H-z-index-1000000 H-height-50 H-width-50 H-border-radius-5 H-theme-background-color-black H-theme-font-color-white H-center-all H-text-align-center H-background-color-transparent-5 animated " + animateName);
